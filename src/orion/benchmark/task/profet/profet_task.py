...
def get_task_network(
    ...
    task_features_mean, task_features_std = get_features(X, Y, C, hidden_space[benchmark],)
...
class ProfetTask(BaseTask, Generic[InputType]):
    def __init__(
        ...
        super().__init__(
            rng=rng, seed=seed, max_trials=max_trials,
        )
        self.task_id = task_id or 0
        self.benchmark = benchmark
        if not input_dir:
            input_dir = Path(os.environ.get("DATA_DIR", "data")) / "profet"
        else:
        ...
        task_hash_params = dict(
            benchmark=benchmark, task_id=task_id, rng=rng, seed=seed, **asdict(self.train_config),
        )
        ...
        if os.path.exists(checkpoint_file):
            ...
            print(
                f"Task hash params:",
                json.dumps({k: str(v) for k, v in task_hash_params.items()}, indent="\t"),
            )
    ...
    def hash(self) -> str:
        ...
        return compute_identity(
            benchmark=self.benchmark, task_id=self.task_id, **self.fixed_values, seed=self.seed,
        )
    ...
    def __call__(self, x: np.ndarray = None, **kwargs):
        ...
        logger.debug(f"received x={x}, kwargs: {kwargs}, fixed values: {self.fixed_values}")
