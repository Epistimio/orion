...
class Sampler:
    """
    Samples trial parameters from an orthogonal Latin hypercube.
    """
    ...
    def generate_olh_samples(space, n_levels, strength, index, rng):
        ...
        if not Sampler._is_prime(n_levels):
            n_levels = Sampler._find_n_levels(dimensions)
            logger.warning(
