...
TreeNode is a generic class which can carry arbitrary python objects. It comes we basic methods to
...
class TreeNode(object):
    ...
    def set_parent(self, node):
        ...
        if node is not None and not isinstance(node, TreeNode):
            raise ValueError("Cannot set parent to %s" % str(node))
    ...
    def add_children(self, *nodes):
        ...
        for child in nodes:
            if child is not None and not isinstance(child, TreeNode):
                raise ValueError("Cannot add %s to children" % str(child))
