...
# FIXME: Fix this
from orion.core.worker.primary_algo import SpaceTransformAlgoWrapper as PrimaryAlgo

from .algo_wrapper import AlgoWrapper, Point, Results

log = getLogger(__file__)

from .knowledge_base import AbstractKnowledgeBase, ExperimentInfo
...
class MultiTaskAlgo(AlgoWrapper):
    ...
    def observe(self, points: list[Point], results: list[float] | list[dict]) -> None:
    ...
    def warm_start(self, warm_start_trials: dict[Mapping, list[Trial]]) -> None:
        ...
        # TODO: Check that the task dim doesn't interfere with this:
        new_trials, new_points, new_task_ids = list(
            zip(
                *[
                    ...
                    if self.get_id(point) not in self.unwrapped._warm_start_trials
                ]
            )
        )
        if not new_trials:
    ...
    def score(self, point: tuple | np.ndarray) -> float:
    ...
    def judge(self, point: Point, measurements: Any) -> dict | None:
    ...
    def get_task_id_from_point(self, point: tuple | np.ndarray) -> int:
