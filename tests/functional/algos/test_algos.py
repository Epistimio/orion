...
hyperband_config = ['hyperband.yaml']
...
@pytest.mark.usefixtures("clean_db")
@pytest.mark.usefixtures("null_db_instances")
@pytest.mark.parametrize('config_file', hyperband_config)
def test_hyperband(database, monkeypatch, config_file):
    """Test a scenario with hyeprband."""
    monkeypatch.chdir(os.path.dirname(os.path.abspath(__file__)))
    orion.core.cli.main(["hunt", "--config", config_file,
                         "./black_box.py", "-x~uniform(-50, 50, precision=None)",
                         "--fidelity~fidelity(1,10,4)"])
