#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Example usage and tests for :mod:`metaopt.client.manual`."""

import pytest

from metaopt.client import insert_trials
...
def test_good_insertion_exp_exists(monkeypatch, database, random_dt):
    ...
    monkeypatch.setenv('METAOPT_DB_NAME', 'metaopt_test')
    monkeypatch.setenv('METAOPT_DB_ADDRESS', 'mongodb://user:pass@localhost')
...
def test_bad_insertion_exp_exists_ignores(database, random_dt):
    ...
    """
    config = dict(database={'name': 'metaopt_test',
                            'type': 'mongodb',
