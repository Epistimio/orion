...
class Experiment {
    ...
    public async update(): Promise<boolean> {
        console.log("Experiment update")
        const profilePromise = axios.get(`/public/experiment.json`);
        const statusPromise = axios.get(`/public/check-status.json`);
        const [ profileResponse, statusResponse ] = await Promise.all([ profilePromise, statusPromise ]);
        let updated = false;
        if (statusResponse.status === 200) {
            console.log("Experiment get check-status.json")
            updated = JSON.stringify(this.statusField) === JSON.stringify(statusResponse.data);
            this.statusField = statusResponse.data;
        }
        if (profileResponse.status === 200) {
            console.log("Experiment get experiment.json")
