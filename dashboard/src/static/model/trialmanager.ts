...
class TrialManager {
    ...
    public async init(): Promise<void> {
        console.log("Trial Manager init")
        while (!this.infoInitialized || !this.metricInitialized) {
            await this.update();
        }
    }
    //<JUN> update trial information from rest server
    public async update(lastTime?: boolean): Promise<boolean> {
        console.log("Trail Manager updates")
    ...
    //<JUN> pool trail information and fill into trial table
    private async updateInfo(): Promise<boolean> {
        const response = await axios.get(`/public/trial-jobs.json`);
        let updated = false;
    ...
    private async updateAllMetrics(): Promise<boolean> {
        const response = await axios.get(`/public/metric-data.json`);
        console.log(response)
